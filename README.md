...
## Stability analysis
No stability analysis is provided in the paper ``Heterogeneous risk tolerance, in-groups, and epidemic waves`` because such analysis was beyond the scope of the conceptual analysis presented. For the interested reader, however, we provide in the file ``frontiers_Stability_Analysis.pdf`` (in the repository ``Stability``) a draft on stability analysis that was finaly not submitted with the paper (hence the content of this stability analysis was not peer reviewed).
