...
### Scenarios for Profile 0 (population with linear reaction to prevalence only)
Scenarios0 <- BSEIR::get.sim.scenarios(b0.vec = 0,
                                       v.vec = 0,
                                       d0.vec = 0,
                                       e.vec = 0,
                                       ...
                                       sep = "+")
write.csv(Scenarios0, file = paste0(path0, "/Scenarios0", ".csv"), row.names = FALSE)

Scenarios0_params.mat <- BSEIR:::scenario2params (Scenarios0, sep = "+",
                                                  name.cols = TRUE)

dim(Scenarios0_params.mat)
head(Scenarios0_params.mat[, 1:13], 20)
