...
### Scenarios for Profile C (population with quadratic effect of rate of change)
ScenariosC <- BSEIR::get.sim.scenarios(b0.vec = 0,
                                       v.vec = 0,
                                       e.vec = 0,
...
write.csv(ScenariosC, file = paste0(path0, "/ScenariosC", ".csv"), row.names = FALSE)

ScenariosC_params.mat <- BSEIR:::scenario2params (ScenariosC, sep = "+",
                                                  name.cols = TRUE)

dim(ScenariosC_params.mat)
head(ScenariosC_params.mat[, 1:13], 50)
tail(ScenariosC_params.mat[, 1:13], 50)
